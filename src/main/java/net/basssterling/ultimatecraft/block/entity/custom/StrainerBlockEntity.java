...
public class StrainerBlockEntity extends BlockEntity implements ExtendedScreenHandlerFactory<BlockPos>, ImplementedInventory {
    ...
    public StrainerBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.STRAINER_BE, pos, state);
        this.propertyDelegate = new PropertyDelegate() {
            @Override
            public int get(int index) {
                return switch (index) {
                    case 0 -> StrainerBlockEntity.this.progress;
                    case 1 -> StrainerBlockEntity.this.maxProgress;
                    default -> 0;
                };
            }
            ...
            public void set(int index, int value) {
                switch (index) {
                    case 0: StrainerBlockEntity.this.progress = value;
                    case 1: StrainerBlockEntity.this.maxProgress = value;
                }
            }
    ...
    public Text getDisplayName() {
        return Text.translatable("block.ultimatecraft.strainer");
    }

    @Nullable
    @Override
    public ScreenHandler createMenu(int syncId, PlayerInventory playerInventory, PlayerEntity player) {
        return new StrainerScreenHandler(syncId, playerInventory, this,this.propertyDelegate);
    }
    ...
    protected void writeNbt(NbtCompound nbt, RegistryWrapper.WrapperLookup registryLookup) {
        ...
        nbt.putInt("strainer.progress", progress);
        nbt.putInt("strainer.max_progress", maxProgress);
    }

    @Override
    protected void readNbt(NbtCompound nbt, RegistryWrapper.WrapperLookup registryLookup) {
        Inventories.readNbt(nbt, inventory, registryLookup);
        progress = nbt.getInt("strainer.progress");
        maxProgress = nbt.getInt("strainer.max_progress");
        super.readNbt(nbt, registryLookup);
    }

    public void tick(World world, BlockPos pos, BlockState state, BlockEntity blockEntity, StrainerBlockEntity be) {
        if(hasRecipe()) {
